...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($cart_items as $item) {
        ...
        $transaction_details['total_amount'] += $discount_price;
        
        // Prepare update for inventory
        $filter = ['Name' => $item['name']];
        $update = ['$inc' => ['Quantity' => -$item['quantity']]]; // Decrease quantity   
        $bulk->update($filter, $update);// Add update operation to bulk write
       ...
       // Execute bulk write to update inventory and lowStock
    ...
    // echo '<script> window.open("bill-format.php", "_blank");</script>';
    // header("Location: index.php");
    echo '<script>
    window.onload = function() {
        window.open("bill-format.php", "_blank"); // Open bill-format.php in a new tab
        window.location.href = "billing.php"; // Redirect current tab to index.php
    };
</script>';
exit;
}
        ...
        <table border='1'>
            <thead>
                <tr style='background-color: #4CAF50; color: white;'>
                    <th>Name</th><th>Type</th><th>Batch No</th><th>Expiry Date</th><th>Quantity Selected</th><th>MRP</th><th>Total Price</th><th>Action</th></tr></thead><tbody><?php 
                    foreach ($cart_items as $index => $item): 

                        echo "<tr><td>" . htmlspecialchars($item['name']) . "</td><td>" . htmlspecialchars($item['type']) . "</td><td>" . htmlspecialchars($item['batch_no']) . "</td><td>" . htmlspecialchars($item['expiry_date']) . "</td><td>" . htmlspecialchars($item['quantity']) . "</td><td>" . htmlspecialchars($item['price']) . "</td><td>" . htmlspecialchars($item['quantity'] * $item['price']) . "</td>";
